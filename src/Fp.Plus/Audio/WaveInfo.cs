...
public record WaveInfo
{
    ...
    /// <summary>
    /// Creates a new instance of <see cref="WaveInfo"/>.
    /// </summary>
    ...
    /// of the read of the subchunk following this
    /// number.</param>
    /// <param name="fact">"fact" chunk body.</param>
    public WaveInfo(int subChunk1Size, short audioFormat, short numChannels, int sampleRate, int byteRate,
    ...
    /// <summary>
    /// Creates a "fact" chunk body.
    /// </summary>
    /// <param name="numSamples">Number of samples.</param>
    /// <param name="contentSize">Content size.</param>
    /// <returns>Content body of fact chunk.</returns>
    /// <exception cref="ArgumentException">Thrown if <paramref name="contentSize"/> is less than 4.</exception>
    ...
    /// <summary>
    /// Creates a new instance of <see cref="WaveInfo"/> for integer PCM data.
    /// </summary>
    /// <param name="numChannels">Mono = 1, Stereo = 2, etc.</param>
    /// <param name="sampleRate">8000, 44100, etc.</param>
    /// <param name="bitsPerSample">8 bits = 8, 16 bits = 16, etc.</param>
    /// <param name="numSamples">Number of samples (shared count between channels).</param>
    public static WaveInfo CreateInteger(short numChannels, int sampleRate, short bitsPerSample, int numSamples)
    ...
    public static unsafe WaveInfo CreateInteger<T>(short numChannels, int sampleRate, int numSamples) where T : unmanaged
    {
    ...
    /// <summary>
    /// Creates a new instance of <see cref="WaveInfo"/> for float PCM data.
    /// </summary>
    /// <param name="numChannels">Mono = 1, Stereo = 2, etc.</param>
    /// <param name="sampleRate">8000, 44100, etc.</param>
    /// <param name="numSamples">Number of samples (shared count between channels).</param>
    /// <typeparam name="T">Sample type.</typeparam>
    public static unsafe WaveInfo CreateFloat<T>(short numChannels, int sampleRate, int numSamples) where T : unmanaged
    {
        short bitsPerSample = (short)(sizeof(T) * 8);
        return new WaveInfo(0x10, 3, numChannels, sampleRate, sampleRate * numChannels * bitsPerSample / 8,
            (short)(numChannels * bitsPerSample / 8), bitsPerSample, 0, null,
